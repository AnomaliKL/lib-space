...
@section('content')
<div class="space-y-6" x-data="{ openAddModal: false, openEditModal: false, openDeleteModal: false, currentBook: {id: '', title: '', author: '', category: 'Framework', stock: 0}, deleteUrl: '' }">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button @click="openAddModal = true" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg shadow-sm flex items-center space-x-2 transition self-start md:self-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col justify-between transition duration-200 hover:shadow-md">
            <div class="p-5 space-y-4">
                <div class="h-40 rounded-lg flex items-center justify-center p-4 text-center select-none
                    {{ $book->category === 'Framework' ? 'bg-gradient-to-br from-blue-500 to-indigo-600' : '' }}
                    {{ $book->category === 'Frontend' ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : '' }}
                    {{ $book->category === 'Database' ? 'bg-gradient-to-br from-amber-500 to-orange-600' : '' }}">
                    <span class="text-white font-bold text-lg tracking-wide shadow-sm">{{ $book->title }}</span>
                </div>
                
                <div>
                    <span class="px-2.5 py-0.5 text-xs font-semibold rounded-full 
                        {{ $book->category === 'Framework' ? 'bg-blue-50 text-blue-700 border border-blue-100' : '' }}
                        {{ $book->category === 'Frontend' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : '' }}
                        {{ $book->category === 'Database' ? 'bg-amber-50 text-amber-700 border border-amber-100' : '' }}">
                        {{ $book->category }}
                    </span>
                    <h3 class="text-lg font-bold text-gray-800 mt-2 line-clamp-1">{{ $book->title }}</h3>
                    <p class="text-xs text-gray-400 mt-1">Oleh: <span class="font-medium text-gray-600">{{ $book->author }}</span></p>
                </div>
            </div>
            ...
            <div class="p-5 pt-0 border-t border-gray-50 flex items-center justify-between mt-auto">
                ...
                <div class="flex space-x-1.5">
                    <button @click="currentBook = {id: '{{ $book->id }}', title: '{{ $book->title }}', author: '{{ $book->author }}', category: '{{ $book->category }}', stock: '{{ $book->stock }}'}; openEditModal = true" class="px-2.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded transition shadow-sm">
                        Edit
                    </button>
        ...
        <div class="col-span-1 sm:col-span-2 lg:col-span-3 bg-white border border-dashed border-gray-200 rounded-xl p-12 text-center text-gray-400">
            Belum ada koleksi buku di dalam database. Silakan klik "+ Tambah Buku".
        </div>
        @endforelse
    </div>
    ...
    <template x-teleport="body">
        <div x-show="openAddModal" class="fixed inset-0 z-[99999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto" style="display: none;">
            <div x-show="openAddModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md" @click="openAddModal = false"></div>

            <div x-show="openAddModal" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-gray-100 relative z-10 my-8">
                ...
                <form action="{{ route('admin.katalog.store') }}" method="POST" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Kategori / Rak</label>
                            <select name="category" class="w-full text-sm bg-gray-50 border border-gray-300 rounded-xl p-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none text-gray-700">
    ...
    <template x-teleport="body">
        <div x-show="openEditModal" class="fixed inset-0 z-[99999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto" style="display: none;">
            <div x-show="openEditModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md" @click="openEditModal = false"></div>

            <div x-show="openEditModal" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-gray-100 relative z-10 my-8">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                ...
                <form :action="'/admin/katalog/' + currentBook.id" method="POST" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="category" x-model="currentBook.category" class="w-full text-sm bg-gray-50 border border-gray-300 rounded-xl p-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none text-gray-700">
                                <option value="Framework">Framework</option>
                                <option value="Frontend">Frontend</option>
                                <option value="Database">Database</option>
                            </select>
    ...
    <template x-teleport="body">
        <div x-show="openDeleteModal" class="fixed inset-0 z-[99999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto" style="display: none;">
            <div x-show="openDeleteModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md" @click="openDeleteModal = false"></div>

            <div x-show="openDeleteModal" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-gray-100 relative z-10 text-center space-y-4 my-8">
